...
class MySqliteRequest
  ...
  def values(data)
    @insert_values = data
    self if @type_of_request == :insert
  end
  ...
  def run_select
    ...
    if @join_table
      ...
      data.each do |row|
        if @where_conditions.all? { |column, value| row[column] == value }
  ...
  def run_insert
    CSV.open(@table_name, 'a+', headers: true) do |csv|
      ...
      csv << @insert_values.values if csv.count.zero?
    end
  end
  ...
  def run_update
    ...
    data.each do |row|
      if @where_conditions.all? { |column, value| row[column] == value }
        @insert_values.each { |key, val| row[key] = val }
      end
    end
    data
  end
  ...
  def run_delete
    data = CSV.table(@table_name, headers: true)
    data.delete_if { |row| @where_conditions.any? { |column, value| row[column] == value } }
    data
  end
end
